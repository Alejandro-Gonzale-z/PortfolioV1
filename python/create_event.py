...
def get_credentials():
    ...
    if os.path.exists(r"python\CREDENTIALS\token.json"):
        #load credentials from file if it exists
        creds = Credentials.from_authorized_user_file(r"python\CREDENTIALS\token.json")

    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(r"python\CREDENTIALS\credentials.json", SCOPES)
            creds = flow.run_local_server(port=0)

        with open(r"python\CREDENTIALS\token.json", "w") as token:
